...
def load_nested_imagenet(model):
    ...
    for layer in model.layers:
        if 'resnet50' in layer.name:
            # print('resnet50 len(layer.layers)', len(layer.layers))
...
def roiconvmulspect(x1, x2, rois_1, num_rois, pooling_regions):
...
#REMOVE THIS SECOND
    out_roi_pool_x1_r1 = RoiPoolingConv(pooling_regions, num_rois)([x1, rois_1])
    out_roi_pool_x2_r1 = RoiPoolingConv(pooling_regions, num_rois)([x2, rois_1])
    merged_rois_r2 = keras.layers.concatenate([out_roi_pool_x1_r1, out_roi_pool_x2_r1])
...
def classifier(base_layers_x1, base_layers_x2, input_rois_1, num_rois, nb_classes=2, trainable=False):
    ...
    out_roi_pool_1,out_roi_pool_2 = roiconvmulspect(base_layers_x1, base_layers_x2, input_rois_1, num_rois, pooling_regions)
    #out_roi_pool_1, = roiconvmulspect(base_layers_x1, base_layers_x2, input_rois_1, num_rois, pooling_regions)
    ...
    out_class_2 = TimeDistributed(Dense(nb_classes, activation='softmax', kernel_initializer='zero'),name='dense_class_2{}'.format(nb_classes))(out_2)
    out_regr_2 = TimeDistributed(Dense(4 * (nb_classes - 1), activation='linear', kernel_initializer='zero'),name='dense_regress_2{}'.format(nb_classes))(out_2)

    return [out_class_1, out_regr_1, out_class_2, out_regr_2]
    #return [out_class_1, out_regr_1]
